...
# --- dot clean: flags, counts, and dry-run (folded from dot link clean, #51) -----

@test "clean leaves a foreign broken symlink under ~/.config alone" {
...
@test "clean removes a deleted-source home dangle (git-history derived)" {
  setup_deleted_source_dangle
  [ -L "$HOME/.claude/settings.json" ] # dangle exists before
  run "$DOT" clean
  [ "$status" -eq 0 ]
...
@test "clean --dry-run counts a broken managed link once and removes nothing" {
  ...
  run "$DOT" clean --dry-run
  [ "$status" -eq 0 ]
  [[ "$output" == *"1 broken symlink"* ]]
  [ -L "$XDG_CONFIG_HOME/demo" ] # dry run removes nothing
}

@test "link clean is no longer a recognized command (#51)" {
  run "$DOT" link clean
  [ "$status" -ne 0 ]
  [[ "$output" == *"Package clean not found in config directory"* ]]
}
