...
- **`dot doctor`** — health check (config links, shell, plugins, tools, drift summary, git-identity audit); `--strict` fails on any warning. The git-identity audit is a report-only sweep over conventional roots (`~/work ~/workspace ~/dev ~/dotfiles ~/personal ~/clients`, plus any dirs set via `dot profile set-config git_audit_roots`, which augment the defaults): it lists repos whose `origin` is on a known host with no matching slot (with the exact `dot git use` / `add-identity` fix) and warns on `gh`-account drift. It never writes and never gates the exit code — the Identity Guard is the enforcer
