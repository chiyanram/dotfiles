...
- **`dot git <setup|add-identity|register-key|use|migrate|remove-identity|install-guard>`** — git identity slots: `setup` the personal fallback, `add-identity` builds a slot (SSH key + host alias + remote-bound, SSH-signed email) and auto-registers the slot key on the host via `register-key`, `register-key --host <host>` trusts the host key and uploads your SSH key to that host's account as BOTH an authentication and a signing key using the right CLI (`gh` for github.\*, `glab` for gitlab.\*) — account-scoped so no repo is needed, and it falls back to print-and-paste when no CLI is authenticated, `use <slot>` binds the current repo to a slot (rewrites `origin` to the slot alias so key/email/signing follow, and switches the `gh` account for GitHub slots), `migrate` interactively onboards existing repos under the audit roots (same scan as `dot doctor`): per mis-set repo it offers a slot and rebinds `origin` on confirm — git-side only, no `gh` switch (re-runnable, non-destructive; run `use` in a repo later to align `gh`), `remove-identity <slot>` tears a slot down symmetrically (SSH alias, gitconfig fragment, `hasconfig` include, `allowed_signers` entry, and — if the key was generated by `add-identity` rather than adopted via `--key` — the key pair itself), warns if any local repo under the audit roots still points at the alias, and reminds you to revoke the key on the host account, `install-guard` activates the Identity Guard globally (points `core.hooksPath` at the committed `pre-commit` hook via `~/.gitconfig-local`) so a commit whose remote is on a known host with no matching slot is blocked with the exact fix command — while any repo-local `pre-commit` hook is still run
