...
- **Work in a worktree by default.** Concurrent Claude sessions run on this repo — do all branch work in an isolated worktree (`.claude/worktrees/`); the shared checkout stays on `main` and is never branch-switched. Inside a worktree the test gate is `./bin/dot-test` (see Tests). **Exception:** a pure prose/comment change to an existing doc (README.md, CLAUDE.md, CONTEXT.md, an ADR — no scripts, tests, or config) may skip the worktree and branch directly in the shared checkout, since there's no executable/test risk. Do it as one continuous flow — branch, edit, commit, push, PR, `checkout main` — rather than leaving the checkout parked off `main` for any length of time, since that's the actual risk the worktree rule protects against (a crash mid-task, or something else touching the shared checkout while it's mid-branch).
...
- Every script (and `setup.sh`) resolves its own location as `DOTFILES="${DOT_TEST_DOTFILES:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)}"` — always self-locates; `DOT_TEST_DOTFILES` is a test-only escape hatch `.zshenv` never sets, so an ambient `DOTFILES` (which every interactive shell has) can never silently redirect a script to another checkout, e.g. when run from a worktree (#142). `bin/dot`'s own subcommand dispatch mirrors this: it tries `$script_dir/dot-<cmd>` before falling back to `$PATH`, so `dot <cmd>` run from a worktree dispatches to that worktree's own scripts, not a same-named command elsewhere on `$PATH`. A test needing a fixture tree exports `DOT_TEST_DOTFILES`, not `DOTFILES` — except a test that sources a `bin/lib/*.sh` file directly (bypassing any entry-point script's resolution line), which must still set plain `DOTFILES`, since library functions read that bare variable with no resolution logic of their own
...
- `setup_sandbox` overrides `$HOME`, so any tool that derives a cache from `$HOME` will build it **inside** the sandbox — reinstalled every run, and racing `teardown_sandbox`'s `rm -rf` ("Directory not empty"). Pin such a tool's cache outside the sandbox, resolved before the `$HOME` override (`PRE_COMMIT_HOME`, #179). A test asserts the sandbox is left with zero stray files; these bugs only reproduce where `XDG_CACHE_HOME` is unset, i.e. CI and not an interactive shell
